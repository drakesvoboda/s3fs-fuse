...
// From https://stackoverflow.com/questions/180947/base64-decode-snippet-in-c#answer-13935718 
...
unsigned char* base64_decode(std::string const& encoded_string) {
  ...
  unsigned char * bytes = new unsigned char[ret.size() + 1];    // additional room for a trailing '\0'
  std::copy(ret.begin(), ret.end(), bytes);
  bytes[ret.size()] = '\0';
  S3FS_PRN_INFO("[DECODEDVEC: %s]", &ret[0]);
  S3FS_PRN_INFO("[DECODED: %s]", bytes);
